...
int handling_and_or(char *buff, int read, char *first_av);
...
/**
 * handling_and_or - Create blocks of code || && with logical part of execution
 * @buff: first buffer that functions read
 * @read: return of read (open with getline)
 * @first_av: av[0]
 * Return: 0 on success
*/
int handling_and_or(char *buff_semicolon, int read, char *first_av)
{
	int i;
	char **cmds_level2 = parse_user_input(buff, ";");

	for (i = 0; cmds_level2[i] != NULL; i++)
		execute_commands(buff, cmds_level2, cmds_level2[i], read, first_av);

	free_dbl_ptr(cmds_level2);
}
